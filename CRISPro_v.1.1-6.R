...
cutoff <- as.numeric( commandArgs()[ 4 ] )
n.cores <- as.numeric( commandArgs()[ 5 ] )
registerDoParallel( n.cores )
file.rama <- commandArgs()[ 6 ]
file.exe.mutate <- commandArgs()[ 7 ]
output_prefix <- commandArgs()[ 8 ]
option <- as.numeric( commandArgs()[ 9 ] )
if( option == 1 )
{
  file.pdb.prefusion <- commandArgs()[ 10 ]
  prefusion.chain <- commandArgs()[ 11 ]
  file.pdb.postfusion <- commandArgs()[ 12 ]
  postfusion.chain <- commandArgs()[ 13 ]
} else if( option == 2 )
{
  file.pdb.prefusion <- commandArgs()[ 10 ]
  prefusion.chain <- commandArgs()[ 11 ]
...
{
  file.pdb.postfusion <- commandArgs()[ 10 ]
  postfusion.chain <- commandArgs()[ 11 ]
} else
{
...
# n.cores <- 100
# registerDoParallel( n.cores )
# file.rama <- "rama8000-transpro.data"
# file.exe.mutate <- "mutate.py"
# output_prefix <- "OUTPUT"
# option <- 1
# file.pdb.prefusion <- "RSVF_option-1_prefusion.pdb"
# prefusion.chain <- "A"
# file.pdb.postfusion <- "RSVF_option-1_postfusion.pdb"
# postfusion.chain <- "A"
...
if( option == 1 )
{
  ...
  print( "Calculate torsion angles for PDB using DSSP" )
  ...
  print( "Secondary structure and PHI and PSI angles" )
  ...
  if( length( vec.helix.start ) > 0 )
  {
    ...
    # df.chain.resno.resid.prefusion.phi.psi.angle.helix[ df.chain.resno.resid.prefusion.phi.psi.angle.helix$resno_prefusion %in% as.character( sort( as.vector( c( vec.helix.start, vec.helix.start + 1, vec.helix.end - 1, vec.helix.end ) ) ) ), ]$helix_termini_prefusion <- 1
    # df.chain.resno.resid.prefusion.phi.psi.angle.helix[ df.chain.resno.resid.prefusion.phi.psi.angle.helix$resno_prefusion %in% as.character( sort( as.vector( c( vec.helix.start, vec.helix.start + 1, vec.helix.end ) ) ) ), ]$helix_termini_prefusion <- 1
    df.chain.resno.resid.prefusion.phi.psi.angle.helix[ df.chain.resno.resid.prefusion.phi.psi.angle.helix$resno_prefusion %in% as.character( sort( as.vector( c( vec.helix.start, vec.helix.start + 1, vec.helix.start + 2, vec.helix.end ) ) ) ), ]$helix_termini_prefusion <- 1
  }
  ...
  print( "Run pymol to mutate all positions in the target conformation" )
  ...
  if( length( vec.helix.start ) > 0 )
  {
    ...
    # df.chain.resno.resid.postfusion.phi.psi.angle.helix[ df.chain.resno.resid.postfusion.phi.psi.angle.helix$resno_postfusion %in% as.character( sort( as.vector( c( vec.helix.start, vec.helix.start + 1, vec.helix.end - 1, vec.helix.end ) ) ) ), ]$helix_termini_postfusion <- 1
    # df.chain.resno.resid.postfusion.phi.psi.angle.helix[ df.chain.resno.resid.postfusion.phi.psi.angle.helix$resno_postfusion %in% as.character( sort( as.vector( c( vec.helix.start, vec.helix.start + 1, vec.helix.end ) ) ) ), ]$helix_termini_postfusion <- 1
    df.chain.resno.resid.postfusion.phi.psi.angle.helix[ df.chain.resno.resid.postfusion.phi.psi.angle.helix$resno_postfusion %in% as.character( sort( as.vector( c( vec.helix.start, vec.helix.start + 1, vec.helix.start + 2, vec.helix.end ) ) ) ), ]$helix_termini_postfusion <- 1
  }
  ...
  #==================================================
  # Run pymol to mutate all positions
  print( "Run pymol to mutate all positions in the alternative conformation" )
...
if( option == 1 )
{
  ...
  df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles <- df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes
  ...
  for( i in 1:nrow( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles ) )
  {
        ...
        is.na( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$no_clash_prefusion[ i ] ) )
    {
      vec.compatible.with.prefusion <- c( vec.compatible.with.prefusion,
               ...
               ( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$helix_prefusion[ i ] == 0 | 
                 (  df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$helix_prefusion[ i ] == 1 & df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$helix_termini_prefusion[ i ] == 1 ) ) &
  ...
  for( i in 1:nrow( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles ) )
  {
        ...
        is.na( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$no_clash_postfusion[ i ] ) )
    {
      vec.compatible.with.postfusion <- c( vec.compatible.with.postfusion,
               ...
               ( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$helix_postfusion[ i ] == 0 | 
                 (  df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$helix_postfusion[ i ] == 1 & df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles$helix_termini_postfusion[ i ] == 1 ) ) &
  ...
  colnames( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC ) <- c( "resno",
                                                                                                                   "chain_targetConformation",
                                                                                                                   "amino_acid_targetConformation",
                                                                                                                   "phi_targetConformation",
                                                                                                                   "psi_targetConformation",
                                                                                                                   "angle_trans_proline_targetConformation",
                                                                                                                   "helix_targetConformation",
                                                                                                                   "helix_termini_targetConformation",
                                                                                                                   "DSSP_targetConformation",
                                                                                                                   "no_clash_targetConformation",
                                                                                                                   "chain_alternativeConformation",
                                                                                                                   "amino_acid_alternativeConformation",
                                                                                                                   "phi_alternativeConformation",
                                                                                                                   "psi_alternativeConformation",
                                                                                                                   "angle_trans_proline_alternativeConformation",
                                                                                                                   "helix_alternativeConformation",
                                                                                                                   "helix_termini_alternativeConformation",
                                                                                                                   "DSSP_alternativeConformation",
                                                                                                                   "no_clash_alternativeConformation",
                                                                                                                   "compatible_targetConformation",
                                                                                                                   "compatible_alternativeConformation",
                                                                                                                   "compatible_targetConformation_NotCompatible_alternativeConformation" )
  
  # df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC <- df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC[ , c( 1:7, 9:16, 18:22 ) ]
...
} else if( option == 2 )
{
  ...
  colnames( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC ) <- c( colnames( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles ),
                                                                                                                   "compatiblePrefusion" )
  colnames( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC ) <- c( "chain_targetConformation",
                                                                                                                   "resno_targetConformation",
                                                                                                                   "amino_acid_targetConformation",
                                                                                                                   "phi_targetConformation",
                                                                                                                   "psi_targetConformation",
                                                                                                                   "angle_trans_proline_targetConformation",
  ...
  # df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC <- df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC[ , c( 1:7, 9:11 ) ]
  #==================================================
  #==================================================
  # Save data frame
  write.csv( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC,
...
} else if( option == 3 )
{
  ...
  colnames( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC ) <- c( colnames( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles ),
                                                                                                                   "compatiblePostfusion" )
  colnames( df.chain.resno.resid.prefusion.postfusion.phi.psi.angle.helix.noclashes.distangles.criteriaGYC ) <- c( "chain_alternativeConformation",
                                                                                                                   "resno_alternativeConformation",
                                                                                                                   "amino_acid_alternativeConformation",
                                                                                                                   "phi_alternativeConformation",
                                                                                                                   "psi_alternativeConformation",
                                                                                                                   "angle_trans_proline_alternativeConformation",
